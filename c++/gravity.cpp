...
void timer(int)
{
  ...
  if (PRESSED_LEFT && !SPEED_PARTICLES) {
    addParticle(10, /*3*/12); //add tiny particle
    PRESSED_LEFT = false;
  }
  ...
  for (i = 0; i < (int)particles.size(); i++) {
    ...
    for (j = 0; j < (int)particles.size(); j++) {
      if (j == i || p.m >= 10000)    continue; // we consider the 10000 as infinit (big mass) so this particles won't move

      const Particle &p1 = particles[j];

      float d = sqrt((p1.x - p.x)*(p1.x - p.x) + (p1.y - p.y)*(p1.y - p.y));
      if (d > p1.r) {
        p.vx += /*0.03*/0.3 * p1.m / (d*d) * (p1.x - p.x)/d; //f = ma => a = f/m
        p.vy += /*0.03*/0.3 * p1.m / (d*d) * (p1.y - p.y)/d;
      }
      else {
        not_fall = false;
      }
...
void mouse(int button, int state, int x, int y)
{
  ...
  if    (button == GLUT_LEFT_BUTTON)  PRESSED_LEFT  = state == GLUT_DOWN;
  else if  (button == GLUT_RIGHT_BUTTON)  PRESSED_RIGHT  = state == GLUT_DOWN;
  else if  (button == GLUT_MIDDLE_BUTTON)  PRESSED_MIDDLE  = state == GLUT_DOWN;
}
