...
CvxNode*
cvx_node_new (CvxField* field, gint x, gint y)
{
  CvxNode* retvar;
...
void
cvx_node_render (CvxNode* node, cairo_t* cr)
{
...
gboolean
cvx_node_is_inside_p (CvxNode* node, gint x, gint y)
{
  double ulx = node->x - node->width / 2.0;
  double uly = node->y - node->height / 2.0;
...
void
cvx_node_move_to (CvxNode* node, gint x, gint y)
{
  node->x = x + node->dx;
  node->y = y + node->dy;
}
...
void
cvx_node_set_difference (CvxNode* node, gint x, gint y)
{
  node->dx = node->x - x;
  node->dy = node->y - y;
}
